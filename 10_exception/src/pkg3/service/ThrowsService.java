...
public class ThrowsService {
	 ...
	 * 1) 경고의 의미
	 * 	-> 해당 메서드 기능을 활용 하다보면 
	 * 	   특정 예외가 발생할 수도 있으니 대비를 하라는 뜻
	 * 2) 예외 처리를 한 곳에서 묶어 처리하기 위하여
	 * 	-> 실질적으로 예외를 처리하는 try - catch 구문이
	 *	   사방에 흩어져 있을 경우
	 *	   돌려받는 최종 결과가
	 *	   정상 결과인지, 예외 처리된 결과인지 구분하기 힘들어짐
	 * 
	 */
	...
	/* 
	 * test4() 호출 시 던져져 온ㄹ 수 있는
	 * IOException을 대비한 코드를 작성
	 * 
	 * 대비 방법 1) try-catch 작성
	 * 			  -> 해당 메서드에서 예외를 처리해 없애버림
	 ...
	 * 해당 메서드를 호출하는 곳에서는
	 * IOException에 대한 예외 처리구문을 작성 해야만 한다
	 * @param num
	 * @throws IOException
	 */
	...
	public void test5() {
		...
		do {
			...
			try {
				...
				input = sc.nextInt(); // 정수가 아닌 값을 입력하면 예외 발생
				sc.nextLine();
	 ...
	 * - Scanner 가 아닌 다른 방법
	 * 
	 * @throws IOException : 키보드 입력 시 발생 할 수 있는 문제를 던짐
	 */
	...
	// BufferedReader : 버퍼에 모여있는 내용을 한번에 읽어옴
	...
	// -> readLine() 메서드는 IOException 을 던질 가능성이 있으므로
	//	  예외처리 구문을 "꼭" 작성해 예외 상황에 대비
