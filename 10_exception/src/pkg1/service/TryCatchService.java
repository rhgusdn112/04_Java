...
public class TryCatchService {
	...
	public void test2() {
		...
		try {
			...
			System.out.println("정수만 입력 해줘ㅠㅠ");
	...
	public void test3() {
		...
		try {
			...
			System.out.println("int 범위 정수만 입력해주세요");
	...
	public void test4() {
		...
		// InputMismatchException에 빨간줄(컴파일에러) 이
		// 발생하는 이유  
		...
		public void test5() {
			...
			try {
				...
				System.out.println("RuntimException 예외 처리 완료");
			}catch(Exception e) {
	 ...
	 * 
	 * 1) Check Exception
	 *  - 반드시 예외 처리 구문을 작성해야되는 예외 객체
	 * 	- 예외 처리 구문 미작성 시, 컴파일 에러 발생(빨간줄)
	 * 
	 * 2) Unchecked Exception
	 *  - 예외 처리 구문 작성 여부가 필수가 아닌 선택하는 예외
	 *  - 예외 처리 구문을 작성 하지 않아도 컴파일 에러 발생을 안함 
	 *  - 대부분 Unchecked Exception 은 if 문으로 해결 가능
	 *  	(사용자, 이용자의 실수 방지)
	 */
	public void test6() {
		
		// Unchecked Exception 확인하기
		// -> 컴파일 에러 없음!
