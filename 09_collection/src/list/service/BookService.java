...
public class BookService {
	...
	public BookService() {
		...
		bookList.add(new BookDTO("선재업고 튀어 대본집 세트", "이시은", 45000));
		bookList.add(new BookDTO("더 머니 북","토스", 19800));
	...
	public List<BookDTO> selectTitle(String title) {
		...
		// bookList 모든 요소를 순차 접근
		for(BookDTO book : bookList) {
			// 책 제목에 title 이 포함되어 있을 경우
	...
	public String removeBook(int index) {
		...
		// 1) index 가 bookList 범위 내 인덱스가 맞는지 확인
		if(index < 0 || index >= bookList.size() ) { // 범위 밖
			return null;
		}
		
		// 2) 정상 범위인 경우 index 번 째 요소를 제거 한 후
		// "제거된 책 제목"을 반환
