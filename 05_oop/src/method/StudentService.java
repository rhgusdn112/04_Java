...
public class StudentService {
	...
	public void mainMenu() {
		...
		do {
			...
			case 1:
				...
				System.out.println(std.getName() + "학생 등록 됨");
				break;

			case 2:
				System.out.println("[학생 정보 확인]");

				// static 은 클래스명을 붙여서 호출!!
	...
	private Student registStudent() {
		...
		// student 변수에는
		// 생성된 Student 객체를 참조하는 주소 값이 담겨있다!!!
	...
	private void updateName(Student std) {
		...
		// 'y'가 입력 되었을 때
		std.setName(newName); // 새로운 이름을 std 가 참조하는 객체에 세팅
		System.out.println("=== 이름 변경 완료 !! ===");
		
	}
	
	/**
	 * 자바 공부 수행 - > std 의 자바 역량 변경
	...
	private void studyHtml(Student std) {
		...
		// 100점(Student.MAX_VALUE) 초과 시 Student.MAX_VALUE 대입
				...
				std.setHtml(temp); // 참조하고 있는 학생 객체 java 필드에 대입
