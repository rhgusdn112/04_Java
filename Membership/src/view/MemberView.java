...
public class MemberView {
	...
	public void mainMenu() {
		...
		do {
			...
			try {
				...
				System.out.println("\n ### 숫자만 입력해 주세요 ### \n");
				input = -1; // 첫 반복에서 종료되지 않도록 값을 변경함
			} catch (IOException e) {
				System.out.println("\n ### 입출력 관련 예외 발생 ###\n");
	...
	private int selectMenu() throws NumberFormatException, IOException {
		...
		System.out.println("3. 이름 검색 (동명이인이 있으면 모두 조회)");
		...
		return input; // 반환값이 Integer 이므로 void 가 아닌 int 로 변경하면 반환값 오류가 해결됨
	}
	...
	private void selectAll() {
		...
		// 회원 목록 조회해 반환하는 서비스 호출
		List<Member> memberList = service.getMemberList();

		// 조회된 회원 목록이 없을 경우
		// -> MemberDaoImpl 생성자 코드에 의해서
		// memberList는 "절대!!" null이 될 수 없다!!!!!!!!!!!!!!!
		...
		System.out.printf("%-6s %-6s %10s %6s \n", "[이름]", "[휴대폰 번호]", "[누적 금액]", "[등급]");
		System.out.println("--------------------------------------------------------");
	...
	private void updateAmount() throws IOException {
		...
		// 금액 증가 대상 회원만 참조할 Member 참조변수 선언하기
		Member target = null;
	...
	private void updateMember() throws IOException {
		...
		if (searchList.size() > 1) { // 동명이인이 있을 경우
			System.out.println("\n *** 대상 회원을 선택해 주세요 *** \n");
