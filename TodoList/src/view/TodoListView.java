...
public class TodoListView {
	...
	private int selectMenu() throws NumberFormatException, IOException {
		...
		System.out.println("1. Todo List Full View");   // 전체 TodoList 보기
	...
	//[1] 할일 목록 전체 보기 
	   private void ListAll() {
	      ...
	      for(int i =0; i < TodoList.size(); i++) {
	         ...
	         System.out.printf("[%d] %23s %9s %13s \n",
	...
	private void detailView() throws IOException {
		...
		// 인덱스 번호로 검색하기
		System.out.println("조회할 할일 인덱스 번호 입력 >>>>");
		int index = Integer.parseInt(br.readLine());
		
		// 목록에서 인덱스 번호로 검색 후 결과 반환
		Todo todo = service.detailIndex(index);
		...
		if(todo != null) {
			...
			System.out.println("완료여부 : " + todo.isComplete());
	...
	private void completeTodo() throws IOException {
		...
		String result = service.completeTodo(target);
	...
	private void deleteTodo() throws IOException {
		...
		String result = service.removeTodo(target);
